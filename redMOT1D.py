...
if __name__ == '__main__': 
    ...
    for num_redCombLines in [100,150,200,250]:
        for redPower in [5e-3,8e-3,11e-3,14e-3]:
            for redBeamRadius in [2e-3,7e-3,12e-3,17e-3]:
                ...
                inits = [(x,vx) for x in np.linspace(-17e-3,0,10) for vx in np.linspace(-0.9,0.9,15)] #0.9 m/s is the max speed covering >95% atoms at 720 microK
                ...
                if bool(gravity):
                    file_save = tables.open_file(filename_mac,mode="a",title= "Red MOT simulation, %.i comb"%num_redCombLines)
                else:
                    file_save = tables.open_file(filename_mac,mode="a",title= "Red MOT simulation, %.i comb"%num_redCombLines)
