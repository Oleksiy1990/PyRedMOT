...
class Descriptions(tables.IsDescription):
    ...
    propagationTime = tables.Float64Col()
    timeSteps = tables.Float64Col()




def simulation_function(initial_conditions,beam_radii,powX,powY,powZ,grad,detunings,sim_number):
    """
    ...
    for radX in beam_radii:
        ...
        file_save = tables.open_file("resultsBlueMOT/sim%.ipX%.ipY%.ipZ%.igrad%.i.hdf5"%(sim_number,int(bluePowerX*1e3),int(bluePowerY*1e3),int(bluePowerZ*1e3),\
        ...
        descr_save["propagationTime"] = tStop
        descr_save["timeSteps"] = len(t)
...
if __name__ == '__main__':
    ...
    radiiXYZ = np.array([1,3,5,7,9,11,13,15,17])*1e-3
